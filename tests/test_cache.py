...
class CacheTestCase(BaseTestCase):
    ...
    def add_test_user(self):
        ...
        self.session.post(f"{self.url}/user/add", json={
            "phone": "13309999999",
            "name": "测试删除缓存",
            "user_group_id": 2
        }, headers={
    ...
    def get_new_test_user_token(self, test_token, test_refresh_token):
        ...
        """
        # 测试被修改的用户要重新获取token
        re_register = self.session.get(f"{self.url}/user-group/purview", headers={
            "Authorization": f"bearer {test_token}"
        }).json()

        # 测试被修改用户正常获取新的访问令牌
        new_token = self.session.get(f"{self.url}/oauth/refresh", headers={
    ...
    def test_cache_after_token_expired(self):
        response = self.session.get(f"{self.url}/oauth/refresh", headers={
            "Authorization": f"{self.phone}-6DDEB6569FFD1E6EAF42FDA1BF349205"
        }).json()
    ...
    def test_cache_after_del_user(self):
        ...
        # 用管理员用户删除刚添加的测试用户
        self.session.post(f"{self.url}/user/del", json=dict(phone="13309999999"), headers={
            "Authorization": f"bearer {token}"
        })
